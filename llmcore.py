...
def _sanitize_leading_user_msg(msg):
    """Rewrite the tool_result block in the user message to plain text to avoid orphaned references.
    History uniformly uses Claude content-block format: content is a list of blocks."""
    msg = dict(msg)  # Shallow copy the outer dict
    ...
    for block in content:
        ...
        if block.get('type') == 'tool_result':
            c = block.get('content', '')
            if isinstance(c, list):  # content itself might also be list[{type:text,text:...}]
...
class SiderLLMSession:
    def __init__(self, cfg):
        from sider_ai_api import Session   # No need to install this package if not using sider
    ...
    def ask(self, prompt, stream=False):
        ...
        if stream: return iter([full_text])   # gen has strange empty replies or infinite loops, sider is fast enough
        return full_text   
...
def _fix_messages(messages):
    """Fix messages to comply with Claude API: alternating, tool_use/tool_result paired"""
...
def openai_tools_to_claude(tools):
    ...
    result = []
    for t in tools:
        if 'input_schema' in t: result.append(t); continue  # Already in Claude format
        fn = t.get('function', t)
...
class ToolClient:
    ...
    def _prepare_tool_instruction(self, tools):
        ...
        tool_instruction = f"""
### Interaction Protocol (Strictly Follow, Always Effective)
Please follow these steps to think and act:
1. **Thinking**: First think within the `<thinking>` tags, analyze the current situation and strategy.
2. **Summary**: Output a *very short* highly abstract single-line (<30 words) physical snapshot in `<summary>`, including the new information generated by the previous tool call result + the intention of the current tool call. This content will enter long-term working memory to record key information; outputting descriptions without actual information increment is strictly prohibited.
3. **Action**: If you need to call a tool, please output one (or more) **<tool_use> blocks** after the main text of your reply, and then stop.
Format: ```<tool_use>{{"name": "tool_name", "arguments": {{parameters}}}}</tool_use>```

### Available Tool Library (Mounted, Always Effective)
{tools_json}
"""
        if self.auto_save_tokens and self.last_tools == tools_json:
            tool_instruction = "\n### Tool Library Status: Always effective (code_run/file_read etc), **can be called normally**. The calling protocol remains unchanged.\n"
    ...
    def _parse_mixed_response(self, text):
        ...
        for json_str in json_strs:
            try:
                ...
                self.last_tools = ''   # llm probably forgot the tool schema, provide it again
            except Exception as e:
...
class NativeToolClient:
    THINKING_PROMPT = """
### Action Guidelines (Always Effective)
For each reply, please follow:
1. First analyze the current situation and strategy within <thinking></thinking> tags.
2. In <summary></summary>, output an extremely simple single-line (<30 words) physical snapshot: new info from last result + current intention. This content enters long-term working memory.
3. Only then output the tool call.
""".strip()
